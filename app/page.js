...
async function getArtikel() {
  try {
    ...
    console.log('Fetching from:', `${baseUrl}/api/artikel?limit=6&page=1`);

    const res = await fetch(`${baseUrl}/api/artikel?limit=6&page=1`, {
      cache: 'no-store',
      headers: {
    ...
    return {
      artikel: Array.isArray(data.articles) ? data.articles : [],
      total: data.total || 0,
    };
  } catch (error) {
...
export default async function Beranda() {
  const { artikel, total } = await getArtikel();
